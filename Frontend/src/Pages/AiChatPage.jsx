...
export default function AiChatPage() {
  ...
  // Append message and scroll chat
  ...
  // PDF RAG query
    const askFromPDF = async (question) => {
    try {
        const res = await fetch("http://localhost:5001/api/rag/ask", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  // 1️⃣ Try PDF first
    let reply = await askFromPDF(userMessage);

  // 2️⃣ If PDF answer is null or empty, fallback to Gemini
    if (!reply) {
    reply = await getBotReply(userMessage);
    }
